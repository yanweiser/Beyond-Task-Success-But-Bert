...
def append_dialogue(dialogue, dialogue_atts, dialogue_length, new_questions, question_length, answer_tokens, pad_token):
    """
    ...
    :param dialogue: [Bx200]
    :param dialogue_length: [B]
    :param new_questions: [Bx30]
    :param question_length: [B]
    :param answer_tokens: [B,1]
    :param pad_token: int
    :returns: dialogue: [Bx200], dialogue_length: [B]
    """
    ...
    for qi, q in enumerate(new_questions):
        ...
        if use_cuda:
            updated_dialogue_atts = torch.LongTensor([1 if x else 0 for x in updated_dialogue]).cuda()
        else:
            updated_dialogue_atts = torch.LongTensor([1 if x else 0 for x in updated_dialogue])
        dialogue_atts[qi] = updated_dialogue_atts
        dialogue[qi] = updated_dialogue.unsqueeze(0)

    return dialogue, dialogue_atts, dialogue_length
