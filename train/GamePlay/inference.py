...
if __name__ == '__main__':
    ...
    ### create a dictionary from new vocab to old vocab for oracle.
    with open('data/old_vocab.json') as f:
        old_v = json.load(f)['word2i']
    with open('data/vocab.json') as f:
        new_v = json.load(f)['word2i']
    translation = {}
    for key in old_v:
        translation[new_v[key]] = old_v[key] 
    ...
    with torch.no_grad():
        ...
        for split, dataset in zip(exp_config['splits'], [dataset_test]):
            print(split)
            eval_log = dict()
            ...
            for i_batch, sample in enumerate(dataloader):
                ...
                history_types = to_var(torch.LongTensor(batch_size, 200).fill_(pad_token))
                history_atts = to_var(torch.LongTensor(batch_size, 200).fill_(pad_token))
                for i in range(history_atts.shape[0]):
                    history_atts[i][0] = 1
                ...
                for q_idx in range(dataset_args['max_no_qs']):
                    ...
                    if use_dataparallel and use_cuda:
                        ...
                        encoder_hidden = model.encoder(
                            history=history[mask_ind], 
                            visual_features=avg_img_features[mask_ind], 
                            history_len=history_len[mask_ind],
                            history_types = history_types[mask_ind],
                            history_atts = history_atts[mask_ind]
                            )
                    ...
                    oracle_in = to_var(torch.empty(qgen_out.shape, dtype=torch.long, device='cuda' if use_cuda else 'cpu'))
                    for i in range(qgen_out.shape[0]):
                        for j in range(qgen_out.shape[1]):
                            oracle_in[i][j] = translation[qgen_out[i][j].item()]
                    ...
                    history[mask_ind], history_atts[mask_ind], history_len[mask_ind] = append_dialogue(
                        dialogue=history[mask_ind],
                        dialogue_atts=history_atts[mask_ind],
            ...
            ########## Logging Block ################
                if exp_config['logging']:
                    ...
                    for bidx in range(batch_size):
                        ...
                        eval_log[sample['game_id'][bidx]]['target_cat_str'] = catid2str[str(sample['target_cat'][bidx].data.item())]
                        ...
                        eval_log[sample['game_id'][bidx]]['obj_list'] = sample['objects'][bidx].data[0].tolist()
