                ...
                for idx, mask in enumerate(masks):
                    ...
                    if idx == 0:
                        ...
                        decider_out, qgen_out = model(
                            ...
                            history_types = sample['history_types'][mask],
                            history_atts = sample['history_atts'][mask])

                        word_logits_loss += _cross_entropy(
                            qgen_out.view(-1, qgen_out.shape[-1]), 
                            sample['target_q'][mask].view(-1).to(torch.long)
                            ) 

                        decider_loss +=  ensemble_args['decider']['ask_weight'] * decider_cross_entropy(
                            decider_out.squeeze(1), 
                            sample['decider_tgt'][mask]
                            )
